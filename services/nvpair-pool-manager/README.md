...
component is phase 1. It does not yet start a donor process, form a pool, or run
inference — it answers the question everything else depends on, which is how much
memory the cluster actually has.
...
One endpoint today, on the cluster port, **cluster mTLS in every state**.
...
cluster state, and the ggml tunnel it will carry in the next phase fronts a
protocol whose only protection is that nothing unpinned can open the socket.

## JSON-RPC methods (broker → manager)

### `pool/status`

This node's donation policy, its own capacity, and the last collected view of its
peers'. Does not re-ask peers.

### `pool/capacity`

The same shape, after re-asking every known peer.

Both answer with `{donor, local, peers, clusterFreeBytes}`. One shape for both,
