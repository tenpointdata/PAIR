...
**Assignment.** Layers go to devices in proportion to effective free VRAM, then
get rounded to integers under a constraint that has nothing to do with memory:
*minimize what crosses the slowest link*. Every layer boundary that falls between
two machines costs a transfer of the hidden state, so the ordering places
consecutive layers on the same device and puts the fewest boundaries on the worst
link.
