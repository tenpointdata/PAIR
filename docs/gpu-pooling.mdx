...
`nvpair-pool-manager` serves that on its own cluster-mTLS port:
...
`reservedBytes` is headroom the node keeps for itself — a desktop that also
renders its own display should not lend its last gigabyte. `committedBytes` is
what an existing pool already claimed, so a second pool does not plan against
memory that is spoken for. It is tracked apart from `usedBytes` because a lease
is a promise made before the memory is touched: between granting one and the
model finishing its load, the driver still reports the memory free.

<Note>
An earlier draft of this design put these fields on `nvpair-node-info`
alongside the hardware inventory. That was wrong, and the implementation moved
them. node-info is deliberately plaintext and unauthenticated — a documented
trade for hostname, GPU model, and utilization — and "this node has 40 GiB
committed to a pool headed by that node" is not hardware inventory. It describes
the shape of a user's cluster and what their machines have promised each other,
which is exactly the class of thing the rule in
[the mesh design](wide-area-mesh.mdx#security) says must never travel in the
clear. node-info still reports only what the driver says.
</Note>

Donation is off until the machine's owner turns it on, and it is refused
entirely while the node belongs to no cluster: there is no pinned peer who may
legitimately ask, so there is nothing to serve.
...
1. **`shared/vrampool` and free-VRAM reporting.** The planner, its cost model,
   ...
   *This exists.* `services/shared/vrampool` implements the cost model, the donor
   policy and its rejection reasons, and the allocator.
   `services/nvpair-pool-manager` is the capacity plane: the donation policy,
   this node's poolable VRAM, the cluster-mTLS surface peers read it from, and
   the collector that gathers the cluster-wide view. It is built and shipped but
   the broker does not supervise it yet, so nothing drives it in a running PAIR.
2. **llama.cpp as an engine,** in `server` mode only. Useful on its own: a third
   engine for GGUF models.

   *Deliberately skipped so far, and worth saying why.* Pooling does not need it:
   `nvpair-pool-manager` runs `rpc-server` and `llama-server` directly from
   command templates, the same way `nvpair-engine-manager` runs an engine from a
   manifest. Making llama.cpp a first-class *routable* engine is a different
   feature — it needs a discovery service key, a proxy that speaks to it, model
   management, and desktop surface — and doing half of that would leave a third
   engine visible in some places and absent in others. It belongs on its own, not
   folded into this.
3. **Donor mode,** including the loopback binding and the mutual-TLS splice.
   This is the security-critical change and deserves its own review.

   *This exists*, in `nvpair-pool-manager` rather than a separate agent — the
   donor and head roles are two halves of one symmetric service, like every other
   peer service in PAIR, and splitting them would have meant two processes
   sharing one lease ledger. A donor grants leases, runs one backend process per
   lease bound to loopback, and splices a cluster-mTLS stream to it; only the
   node the lease was issued to may open that stream. A backend that binds
   anything wider than loopback is detected and killed rather than trusted. What
   is missing is the head half: nothing yet decides which donors to use or starts
   a `llama-server` against them.
4. **`nvpair-pool-manager`:** formation, admission, lifecycle, teardown.

   *This exists.* A head plans across the cluster, leases the donors it needs,
   opens their tunnels, starts a server against them, and renews every lease for
   as long as the pool lives. Admission is `pool/plan`, which reads a model's
   GGUF header — never a tensor — so "does this fit" is answered before gigabytes
   move, and answers `fitsOnOneNode` first, because a model that fits on one node
   should be routed rather than pooled. A formation that fails at any step tears
   down everything it had already brought up.

   Link quality reaches the planner here, from `shared/linkq`: a capacity sweep
   is a timed request that was going to happen anyway, so its round-trip time and
   its outcome are free measurements. The planner uses the result as a gate,
   which is what the failure model demands.
5. **Integration:** pool-as-owner in the proxies, member charging in the
   scheduler, and the interface.

   *Partly.* `nvpair-ui-broker` supervises the pool manager, registers its
   `pl` service so peers can read this node's capacity, projects the discovery
   directory into the peer set the worker collects from, and relays every pool
   method and notification — so pooling is reachable from anything that speaks
   to the broker. The rest of this phase is not done: the proxies do not yet
   present a pool as an advertised owner, the scheduler does not charge a pool's
   work to its members, and nothing in the desktop application shows or
   configures any of it. The llama.cpp command templates are broker flags today,
   which means pooling is configured by whoever launches the broker rather than
   by a user.
